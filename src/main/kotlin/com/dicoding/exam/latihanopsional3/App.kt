...
fun main() {
    println(manipulateString("dicoding123", 10))
    println(manipulateString("Dicoding000010", 100))
    println(manipulateString("dicoding", 100))
}

fun manipulateString(inputString: String, inputNumber: Int): String {
    val digits = inputString.filter { it.isDigit() }

    return if (digits.isNotEmpty()) {
        val numberFromDigits = digits.toBigInteger() * inputNumber.toBigInteger()
        inputString.replace(Regex("\\d+"), numberFromDigits.toString())
    } else {
